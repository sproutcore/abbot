...
JSDOC.Symbol.prototype.init = function() {
  ...
  /* START: Added by pbergstr on 01/15/2009 */
	this.copyright = "";
	this.observes = [];
	this.binding = [];
	...
	this.commonTask = "";
  /* END: Added by pbergstr on 01/15/2009 */
...
JSDOC.Symbol.prototype.setTags = function() {
  ...
  /* START: Added by pbergstr on 01/29/2009 */

	var commonTask = this.comment.getTag("commonTask");
	if (commonTask.length) {
		this.commonTask = commonTask[0].desc;
	}

	var obs = this.comment.getTag("observes");
	if (obs.length) {
		this.observes = obs.map(function($){return $.desc});
	}

	var bind = this.comment.getTag("binding");
	if (bind.length) {
		this.binding = bind.map(function($){return $.desc});
	}


	var isComputed = this.comment.getTag("isComputed");
  if(isComputed.length)
  {
    this.isComputed = true;
  }

	var isReadOnly = this.comment.getTag("isReadOnly");
  if(isReadOnly.length)
  {
    this.isReadOnly = true;
  }

	var rights = this.comment.getTag("copyright");
	if (rights.length) {
		this.copyright = rights.map(function($){return $.desc});
	}


  /* END: Added by pbergstr on 01/29/2009 */
	...
	*/
	...
	// @field
	var field = this.comment.getTag("field");
	if (field.length) 
	{
		this.isa = "OBJECT";
    this.type = field[0].type;
	}
	

  // var binding = this.comment.getTag("binding");
  // if (binding.length) 
  // {
  //  this.isa = "OBJECT";
  //     this.type = binding[0].type;
  //     
  //     // for(var key in this)
  //     // {
  //     //   LOG.warn(key);
